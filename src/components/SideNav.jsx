...
import  {chemicalCategories}  from "../data.js";
      ...
      {hasSubcategories && open && (
        <div>
          {category.subcategories.map((sub, index) => (
            <CategoryItem key={index} category={sub} level={level + 0.5} />
          ))}
        </div>
      )}
    </div>
...
export default function ChemicalSidebar() {
  return (
    <aside className=" h-screen sticky overflow-y-scroll hide-scrollBar top-0 border-r border-gray-200 bg-white p-4 max-sm:w-full">
